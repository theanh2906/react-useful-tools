  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-display font-bold text-white">Ultrasound Gallery</h1>
          <p className="text-slate-400 mt-1">Save and revisit precious moments</p>
        </div>
      ...
      {items.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-white mb-2">No ultrasounds yet</h3>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence mode="popLayout">
            {items.map((item) => (
              ...
              >
                <Card className="p-3">
                  <img src={item.url} alt="ultrasound" className="rounded-xl w-full h-48 object-cover" />
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title="Ultrasound Preview" size="lg">
        {selected && (
          <div className="space-y-4">
            <img src={selected.url} alt="ultrasound" className="rounded-xl w-full object-contain max-h-[480px]" />
            <div className="flex items-center gap-2">
              <Badge variant="primary">{selected.date}</Badge>
              {selected.notes && <Badge variant="default">{selected.notes}</Badge>}
            </div>
          </div>
        )}
        <ModalFooter>
