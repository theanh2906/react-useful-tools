/**
 * @module AuthPage
 * @description Authentication page with email/password sign-in, sign-up,
 * and Google/Microsoft social login.
 */
...
/**
 * Authentication page supporting email login, registration and social providers.
 * After successful login the user is redirected to the `redirect` search param or `/`.
 */
export function AuthPage() {
  ...
  const { setLoading, isLoading, login } = useAuthStore();
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // In Vite, process.env.SECRET_KEY is defined in vite.config.ts
      const secret = (process.env as any).SECRET_KEY;
      if (adminSecret === secret) {
        const adminUser: AppUser = {
          id: 'twNUQtiUn9bUnBFgNhRM3wC64h23',
        ...
        <Card className="p-6 lg:p-8">
          ...
          {isAdminLogin ? (
            <form onSubmit={handleAdminLogin} className="space-y-4">
              <Input
                label="Administrator Secret Key"
                type="password"
