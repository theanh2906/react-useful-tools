...
interface MealCheckInState {
  ...
  // Actions
  loadMonthCheckIns: (userId: string, year: number, month: number) => Promise<void>;
  createCheckIn: (userId: string, date: string, imageFile: File, notes?: string) => Promise<void>;
...
export const useMealCheckInStore = create<MealCheckInState>((set, get) => ({
  ...
  createCheckIn: async (userId: string, date: string, imageFile: File, notes?: string) => {
    set({ isLoading: true, error: null });
    try {
      ...
      if (existingIndex >= 0) {
        ...
        updatedCheckIns = [...currentCheckIns, newCheckIn].sort(
  ...
  deleteCheckIn: async (checkIn: MealCheckIn) => {
    ...
    try {
      ...
      const year = dateObj.getFullYear();
      const month = dateObj.getMonth() + 1;
      const stats = await mealCheckInService.getMonthStats(checkIn.userId, year, month);
